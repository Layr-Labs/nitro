...
### WASI SDK Setup (Required for WebAssembly builds)

The arbitrator components require WASI SDK to compile WebAssembly modules. Follow these steps to install:

**macOS (ARM64):**
```bash
# Download and install WASI SDK
curl -L https://github.com/WebAssembly/wasi-sdk/releases/download/wasi-sdk-25/wasi-sdk-25.0-arm64-macos.tar.gz -o /tmp/wasi-sdk.tar.gz
mkdir -p ~/wasi-sdk
tar -xzf /tmp/wasi-sdk.tar.gz -C ~/wasi-sdk --strip-components=1
...
```bash
# Download and install WASI SDK
curl -L https://github.com/WebAssembly/wasi-sdk/releases/download/wasi-sdk-25/wasi-sdk-25.0-x86_64-macos.tar.gz -o /tmp/wasi-sdk.tar.gz
mkdir -p ~/wasi-sdk
tar -xzf /tmp/wasi-sdk.tar.gz -C ~/wasi-sdk --strip-components=1
...
```bash
# Build replay environment (requires WASI SDK)
PATH="$HOME/wasi-sdk/bin:$PATH" make build-replay-env WASI_SYSROOT=~/wasi-sdk/share/wasi-sysroot

# Or set environment variables permanently in your shell profile
export PATH="$HOME/wasi-sdk/bin:$PATH"
...
**Troubleshooting:**
- If you get "unable to create target: 'No available targets are compatible with triple 'wasm32-unknown-wasip1'", ensure WASI SDK's clang is in your PATH
- The system clang doesn't support WebAssembly targets - you must use WASI SDK's clang
